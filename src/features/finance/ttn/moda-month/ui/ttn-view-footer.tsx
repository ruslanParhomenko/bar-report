...
export default function TtnViewFooterTable({
  rowSuppliers,
  monthDays,
}: {
  rowSuppliers: GetTTNData["ttnData"]["rowSuppliers"];
  monthDays: MonthDayType[];
}) {
  const arrayRows = Object.keys(rowSuppliers);
  ...
  const totals = useMemo(() => {
    return arrayRows.reduce(
      (acc, row) => {
        const rowData = rowSuppliers?.[row];
